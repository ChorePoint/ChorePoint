...
public class GetSubmissionsByParentHandler(
...
) : IRequestHandler<GetSubmissionsByParentQuery, IReadOnlyList<GetSubmissionsByParentResponse>>
{
    ...
    )
    {
        ...
        if (request.Pending)
            query = query.Where(cs => cs.ApprovalStatus.Equals(ChoreApprovalStatus.Pending));
        ...
        if (choreSubmissions.Empty())
        {
            ...
            throw new NotFoundException(
                $"No {pendingText} submissions found with parent ID [{parentId}]"
            );
        }
